      ...
      <SheetContent className="flex flex-col w-full sm:max-w-md p-0">
        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center gap-4 px-6">
            ...
            <div>
              <p className="font-heading font-semibold text-foreground">{t.cart.empty}</p>
              <p className="text-sm text-muted-foreground mt-1">{t.cart.emptyDesc}</p>
            </div>
            <Button variant="hero" size="sm" onClick={handleContinue}>{t.cart.browseShop}</Button>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
              {items.map((item) => (
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-accent/30 border border-border/40 transition-all hover:border-border">
                  <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-primary/10 to-accent flex items-center justify-center text-lg flex-shrink-0">🌿</div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <div className="flex items-center gap-1 border border-border rounded-lg bg-background">
                        <button className="h-7 w-7 flex items-center justify-center hover:bg-accent rounded-l-lg transition-colors" onClick={() => updateQuantity(item.id, item.quantity - 1)}>
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="text-xs font-semibold w-6 text-center">{item.quantity}</span>
                        <button className="h-7 w-7 flex items-center justify-center hover:bg-accent rounded-r-lg transition-colors" onClick={() => updateQuantity(item.id, item.quantity + 1)}>
                  ...
                  <button className="p-1.5 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors" onClick={() => removeItem(item.id)}>
